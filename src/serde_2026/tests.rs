...
fn test_roundtrip_simple_atom() {
    ...
    let deserialized = deserialize_2026(
        &mut new_allocator,
        &serialized,
...
fn test_round_trip_double() {
    ...
    let node2 =
        deserialize_2026(&mut allocator2, &serialized1, DeserializeLimits::default()).unwrap();
...
fn test_auto_detect_classic_format_explicitly() {
    ...
    let mut a2 = Allocator::new();
    let decoded =
        node_from_bytes_auto(&mut a2, &classic_bytes, DeserializeLimits::default()).unwrap();
...
#[test]
fn test_serialized_length_serde_2026_atom() {
    let mut allocator = Allocator::new();
    let node = allocator.new_atom(b"hello").unwrap();
    let bytes = node_to_bytes_serde_2026(&allocator, node).unwrap();
    assert_eq!(
...
#[test]
fn test_serialized_length_serde_2026_pair() {
    let mut allocator = Allocator::new();
    let left = allocator.new_atom(b"left").unwrap();
    let right = allocator.new_atom(b"right").unwrap();
    let pair = allocator.new_pair(left, right).unwrap();
...
#[test]
fn test_serialized_length_serde_2026_with_trailing_data() {
    let mut allocator = Allocator::new();
    let node = allocator.new_atom(b"test").unwrap();
    let bytes = node_to_bytes_serde_2026(&allocator, node).unwrap();
...
#[test]
fn test_serialized_length_serde_2026_complex_tree() {
    let mut allocator = Allocator::new();
...
#[test]
fn test_serialized_length_serde_2026_rejects_non_prefixed() {
