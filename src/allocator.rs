...
impl Allocator {
    ...
    /// Restore to the checkpoint and, if the return value was allocated after the
    /// checkpoint, fix up accounting and optionally produce a replacement node.
    /// Caller must replace the value stack top when `Replace(node)` is returned,
    /// and push the checkpoint back when `Aborted` is returned.
    pub fn maybe_restore_with_node(
        ...
        match self.checkpoint_node_status(checkpoint, ret) {
            ...
            NodeStatus::AfterNewBytes => {
                ...
                self.ghost_atoms -= 1;
                if self.ghost_heap < saved_bytes.len() {
                    return Err(EvalErr::InternalError(
                        NodePtr::NIL,
                        "ghost heap accounting error".to_string(),
                    ));
                }
                self.ghost_heap -= saved_bytes.len();
                Ok(MaybeRestore::Replace(self.new_atom(&saved_bytes)?))
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_maybe_restore_with_node() {
        let mut a = Allocator::new();

        // Before: return value allocated before checkpoint
        let atom1 = a.new_atom(&[4, 3, 2, 1]).unwrap();
        let cp1 = a.transparent_checkpoint();
        let out = a.maybe_restore_with_node(&cp1, atom1).unwrap();
        assert!(matches!(out, MaybeRestore::NoReplace));
        assert_eq!(a.atom(atom1).as_ref(), [4, 3, 2, 1]);

        // AfterOldBytes: atom created after checkpoint but bytes from before
        let atom_hello = a.new_atom(b"hello").unwrap();
        let cp2 = a.transparent_checkpoint();
        let substr = a.new_substr(atom_hello, 0, 5).unwrap();
        assert_eq!(a.atom(substr).as_ref(), b"hello");
        let out = a.maybe_restore_with_node(&cp2, substr).unwrap();
        let MaybeRestore::Replace(new_node) = out else {
            panic!("expected Replace");
        };
        assert_eq!(a.atom(new_node).as_ref(), b"hello");

        // AfterNewBytes: small atom recreated from copied bytes
        let cp3 = a.transparent_checkpoint();
        let atom_x = a.new_atom(b"foobar").unwrap();
        let out = a.maybe_restore_with_node(&cp3, atom_x).unwrap();
        let MaybeRestore::Replace(new_node) = out else {
            panic!("expected Replace");
        };
        assert_eq!(a.atom(new_node).as_ref(), b"foobar");

        // Aborted: atom > 48 bytes cannot be recreated
        let cp4 = a.transparent_checkpoint();
        let big: Vec<u8> = (0..49).collect();
        let atom_big = a.new_atom(&big).unwrap();
        let out = a.maybe_restore_with_node(&cp4, atom_big).unwrap();
