...
impl ReadCacheLookup {
    ...
    pub fn pop2_and_cons(&mut self) {
        // We remove each side of each left/right pair.
        let right = self.pop();
        let left = self.pop();
...
mod tests {
    ...
    fn test_read_cache_lookup() {
        let large_max = 30;
        let mut lookup = ReadCacheLookup::new();

        // The only thing cached right now is a nil, right at the top of the tree (ie. `1`).
        let hash_of_nil = hash_blob(&[1]);
        assert_eq!(lookup.find_paths(&hash_of_nil, large_max), [[1]]);

        assert_eq!(lookup.count.get(&hash_of_nil).unwrap(), &1);

        // The atom `1` is not in the tree anywhere.
        let hash_of_1_atom = hash_blobs(&[&[1], &[1]]);
        assert!(lookup.find_paths(&hash_of_1_atom, large_max).is_empty());

        // Now let's push a `5` atom to the top.
        // Tree: `(5 . 0)`
        let hash_of_5_atom = hash_blobs(&[&[1], &[5]]);
        lookup.push(hash_of_5_atom);
        let hash_of_cons_5_nil = hash_blobs(&[&[2], &hash_of_5_atom, &hash_of_nil]);
        assert_eq!(lookup.find_paths(&hash_of_cons_5_nil, large_max), [[1]]);
        assert_eq!(lookup.find_paths(&hash_of_5_atom, large_max), [[2]]);
        assert_eq!(lookup.find_paths(&hash_of_nil, large_max), [[3]]);

        assert_eq!(lookup.count.get(&hash_of_cons_5_nil).unwrap(), &1);
        assert_eq!(lookup.count.get(&hash_of_5_atom).unwrap(), &1);
        assert_eq!(lookup.count.get(&hash_of_nil).unwrap(), &1);
        ...
        lookup.push(hash_of_9_atom);
        let hash_of_cons_9_cons_5_nil = hash_blobs(&[&[2], &hash_of_9_atom, &hash_of_cons_5_nil]);

        assert_eq!(
            lookup.find_paths(&hash_of_cons_9_cons_5_nil, large_max),
            [[1]]
        );
        assert_eq!(lookup.find_paths(&hash_of_9_atom, large_max), [[2]]);
        assert_eq!(lookup.find_paths(&hash_of_cons_5_nil, large_max), [[3]]);
        assert_eq!(lookup.find_paths(&hash_of_5_atom, large_max), [[5]]);
        assert_eq!(lookup.find_paths(&hash_of_nil, large_max), [[7]]);

        assert_eq!(lookup.count.get(&hash_of_cons_9_cons_5_nil).unwrap(), &1);
        assert_eq!(lookup.count.get(&hash_of_9_atom).unwrap(), &1);
        assert_eq!(lookup.count.get(&hash_of_cons_5_nil).unwrap(), &1);
        assert_eq!(lookup.count.get(&hash_of_5_atom).unwrap(), &1);
        assert_eq!(lookup.count.get(&hash_of_nil).unwrap(), &1);
        ...
        lookup.push(hash_of_10_atom);
        let hash_of_cons_10_cons_9_cons_5_nil =
            hash_blobs(&[&[2], &hash_of_10_atom, &hash_of_cons_9_cons_5_nil]);
        assert_eq!(
            lookup.find_paths(&hash_of_cons_10_cons_9_cons_5_nil, large_max),
            [[1]]
        );
        assert_eq!(lookup.find_paths(&hash_of_10_atom, large_max), [[2]]);
        assert_eq!(
            lookup.find_paths(&hash_of_cons_9_cons_5_nil, large_max),
            [[3]]
        );
        assert_eq!(lookup.find_paths(&hash_of_9_atom, large_max), [[5]]);
        assert_eq!(lookup.find_paths(&hash_of_cons_5_nil, large_max), [[7]]);
        assert_eq!(lookup.find_paths(&hash_of_5_atom, large_max), [[11]]);
        assert_eq!(lookup.find_paths(&hash_of_nil, large_max), [[15]]);

        assert_eq!(
            lookup
                .count
                .get(&hash_of_cons_10_cons_9_cons_5_nil)
                .unwrap(),
            &1
        );
        assert_eq!(lookup.count.get(&hash_of_10_atom).unwrap(), &1);
        assert_eq!(lookup.count.get(&hash_of_cons_9_cons_5_nil).unwrap(), &1);
        assert_eq!(lookup.count.get(&hash_of_9_atom).unwrap(), &1);
        assert_eq!(lookup.count.get(&hash_of_cons_5_nil).unwrap(), &1);
        assert_eq!(lookup.count.get(&hash_of_5_atom).unwrap(), &1);
        ...
        assert_eq!(
            lookup.find_paths(&hash_of_cons_cons_9_10_cons_5_nil, large_max),
            [[1]]
        );
        assert_eq!(lookup.find_paths(&hash_of_cons_9_10, large_max), [[2]]);
        assert_eq!(lookup.find_paths(&hash_of_cons_5_nil, large_max), [[3]]);
        assert_eq!(lookup.find_paths(&hash_of_9_atom, large_max), [[4]]);
        assert_eq!(lookup.find_paths(&hash_of_10_atom, large_max), [[6]]);
        assert_eq!(lookup.find_paths(&hash_of_5_atom, large_max), [[5]]);
        assert_eq!(lookup.find_paths(&hash_of_nil, large_max), [[7]]);
        ...
        assert_eq!(
            lookup
                .count
                .get(&hash_of_cons_cons_9_10_cons_5_nil)
                .unwrap(),
            &1
        );
        assert_eq!(lookup.count.get(&hash_of_cons_9_10).unwrap(), &1);
        assert_eq!(lookup.count.get(&hash_of_cons_5_nil).unwrap(), &1);
        assert_eq!(lookup.count.get(&hash_of_9_atom).unwrap(), &1);
        assert_eq!(lookup.count.get(&hash_of_10_atom).unwrap(), &1);
        assert_eq!(lookup.count.get(&hash_of_5_atom).unwrap(), &1);
        ...
        // `(9 . (5 . 0))` is no longer in the tree.
        assert_eq!(lookup.count.get(&hash_of_cons_9_cons_5_nil).unwrap(), &0);

        // The atom `1` is still not in the tree anywhere.
        assert!(lookup.find_paths(&hash_of_1_atom, large_max).is_empty());

        assert!(lookup.count.get(&hash_of_1_atom).is_none());
    }
}
