                ...
                Operation::RestoreAllocator => {
                    ...
                    match self.allocator.maybe_restore_with_node(&checkpoint, top)? {
                        ...
                        MaybeRestore::Aborted => {
                            self.allocator_stack.push(checkpoint);
                        }
                    }
                    0
                }
...
mod tests {
    ...
    const TEST_CASES: &[RunProgramTest] = &[
        ...
        RunProgramTest {
            ...
            args: "()",
            flags: ClvmFlags::ENABLE_KECCAK_OPS_OUTSIDE_GUARD,
            result: Some("()"),
            cost: 994,
            err: "",
        },
        ...
        RunProgramTest {
            prg: "(sha256tree (q . 10))",
            args: "()",
            flags: ClvmFlags::NO_UNKNOWN_OPS.union(ClvmFlags::ENABLE_SHA256_TREE),
    ...
    #[case::keccak(
        ...
        (1134, 1, ClvmFlags::ENABLE_KECCAK_OPS_OUTSIDE_GUARD),
        ""
    )]
    #[case::keccak(
    ...
    fn test_softfork(
        #[case] prg: &'static str,
        #[case] fields: (u64, u8, ClvmFlags), // cost, enabled, hard_fork_flag
        #[case] err: &'static str,
        #[values(ClvmFlags::empty())] flags: ClvmFlags,
        ...
        let t = RunProgramTest {
            ...
            args: "()",
            flags,
            result: if err.is_empty() && hard_fork_flag == ClvmFlags::empty() {
                Some("()")
            } else {
                None
            },
            cost: cost - 140,
            err: if hard_fork_flag == ClvmFlags::empty() {
                err
            } else if mempool {
                "unimplemented operator"
            } else {
    ...
    fn test_counters() {
        ...
        assert_eq!(counters.allocated_atom_count, 970);
