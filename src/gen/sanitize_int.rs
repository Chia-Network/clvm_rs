...
fn test_count_zeros_and_sanitize_int() {
    ...
    // now do the same thing again, but use sanitize_integer instead of
    // count_zeros()
    {
        ...
        assert!(sanitize_integer(&a, no_leading_zero, 8, e, &mut range_cache, 0).is_err());
        // since we didn't count any leading zeros, we also didn't add anything
        // to the cache
        ...
        assert_eq!(
            sanitize_integer(&a, just_zeros, 8, e, &mut range_cache, 0).unwrap(),
            &[]
        );
        // all of the range [10, 70) were zeros. That should have been added to
        // the cache
        ...
        let a1 = a.new_substr(atom, 1, 101).unwrap();
        assert_eq!(
            sanitize_integer(&a, a1, 8, e, &mut range_cache, 0).unwrap(),
            &[0x7f]
        );
        // now we discovered that [1, 100) are zeros too, which should have been
        // added to the cache
