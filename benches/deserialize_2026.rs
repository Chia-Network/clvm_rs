...
fn deserialize_2026_benchmark(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("deserialize_2026");

    for (name, block) in blocks {
        // Convert legacy format to serde_2026, then benchmark deserialization
...
criterion_group!(benches, deserialize_2026_benchmark);
