...
fuzz_target!(|data: &[u8]| {
    ...
    let non_strict = read_varint(&mut cur, false);

    let Ok(v) = non_strict else {
        // If non-strict failed, strict must also fail (strict is stricter).
        assert!(read_varint(&mut Cursor::new(data), true).is_err());
        return;
    };
    ...
    let strict_canonical = read_varint(&mut Cursor::new(&canonical[..]), true)
    ...
    match read_varint(&mut Cursor::new(data), true) {
